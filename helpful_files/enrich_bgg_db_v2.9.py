...
PROGRESS_EVERY = 200
token = st.secrets.get("BGG_TOKEN", "")
...
# -----------------------
def fetch_bgg_details(game_ids: list, token=token, max_retries: int = 5) -> list:
    """
    ...
    while attempt <= max_retries:
        ...
        for item in root.findall("item"):
            try:
                results.append(parse_thing_item(item))  # your existing parse_thing_item function
...
if __name__ == "__main__":
    main(token=st.secrets.get("BGG_TOKEN", ""))