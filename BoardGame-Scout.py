...
RATINGS_REFRESH_INTERVAL = 8 * 60 * 60  # 8 hours
RATINGS_DB_URL = (
    "https://github.com/"
...
def download_database():
    """
    Download boardgames_db.zip directly from GitHub releases
    if the local database is missing or stale.
    """

    ZIP_URL = (
        "https://github.com/"
        "fotopoulos-v/BoardGame-Scout/"
        "releases/download/boardgame-database/boardgames_db.zip"
    )

    # Re-download if DB is older than this (seconds)
    REFRESH_INTERVAL = 24 * 60 * 60  # 24 hours

    # --- 1. Check local DB age ---
    if os.path.exists(DB_PATH):
        db_age = time.time() - os.path.getmtime(DB_PATH)

        if db_age < REFRESH_INTERVAL:
            print(f"✅ Using existing database (age: {db_age/3600:.1f} hours)")
            return DB_PATH

        print("⬇️ Local database is stale — downloading new version")
    else:
        print("⬇️ No local database found — downloading")

    # --- 2. Download ZIP directly ---
    try:
        print(f"📡 Downloading database from {ZIP_URL}")
        ...
        if os.path.exists(DB_PATH):
            st.warning(f"⚠️ Using existing database (download failed: {e})")
            return DB_PATH
...
def recommend_games(username: str, n: int = RECOMMEND_COUNT) -> tuple[pd.DataFrame, str]:
    ...
    - DataFrame: recommendations or empty
    - status_message: "success", "no_ratings", or "no_similar_users"
    """
    import sqlite3, pandas as pd, numpy as np
    ...
    if user_rates.empty:
        return pd.DataFrame(), "no_ratings"  # ← Changed
    ...
    if neighbours.empty:
        ...
        if neighbours.empty:
            return pd.DataFrame(), "no_similar_users"  # ← Changed
    ...
    return recs[["game_id", "title", "pred", "avg_greek", "reason"]].rename(columns={"pred": "Predicted Rating"}), "success"
    ...
    if recommend_clicked:
        if not username:
            ...
            with st.spinner("Building your personal recommendations..."):
                rec_df, status = recommend_games(username, RECOMMEND_COUNT)  # ← Now unpacks tuple
                st.session_state["rec_df"] = rec_df

            if status == "no_ratings":
                st.warning(
                    f"⚠️ No ratings found for username '{username}'. "
                    "Please rate some games on BoardGameGeek first."
                )
            elif status == "no_similar_users":
                st.warning(
                    f"⚠️ Found ratings for '{username}', but couldn't find "
...
if st.session_state.get("show_search_results"):
    ...
    if total == 0:
        ...
        df_display["Average Rating"] = pd.to_numeric(df_display["Average Rating"], errors='coerce').round(2)
        df_display["Complexity"] = pd.to_numeric(df_display["Complexity"], errors='coerce').round(2)
